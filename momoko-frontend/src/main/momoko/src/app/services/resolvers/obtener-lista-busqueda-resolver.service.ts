...
@Injectable()
export class ObtenerListaBusquedaResolverService implements Resolve<ObtenerPaginaBusquedaResponse> {

  private log = environment.log;

  constructor(private clasificadorService: ClasificadorService, private router: Router) { }

  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<ObtenerPaginaBusquedaResponse> {
    if (this.log) {
      console.log('Obteniendo busqueda');
    }
    const parametrosABuscar = route.paramMap.get('parametros_a_buscar');

    return this.clasificadorService.getBusquedaPage(parametrosABuscar).take(1).map(busqueda => {
