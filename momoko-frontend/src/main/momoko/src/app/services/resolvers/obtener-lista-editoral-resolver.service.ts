...
@Injectable()
export class ObtenerListaEditoralResolverService
  implements Resolve<ObtenerPaginaEditorialResponse> {
  ...
  resolve(
  ...
  ): Observable<ObtenerPaginaEditorialResponse> {
    if (this.log) {
    ...
    if (numeroPagina) {
      return this.clasificadorService.getEditorialPage(url, numeroPagina).pipe(
        take(1),
        map(editorial => {
      ...
      return this.clasificadorService.getEditorial(url).pipe(
        take(1),
        map(editorial => {
          if (editorial.editorial != null) {
            return editorial;
