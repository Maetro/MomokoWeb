...
//	
//	private static final String NONCE = MomokoUtils.uid();
//	
...
//				.content(MomokoUtils.toJson(form(null))))
...
//				.header(MomokoSecurityConfig.TOKEN_REQUEST_HEADER_NAME, token))
//				.andExpect(status().is(401));
//	}
//
//	@Test
...
//		mvc.perform(post("/api/core/login-with-nonce")
//				.contentType(MediaType.APPLICATION_JSON)
//				.content(MomokoUtils.toJson(new NonceForm<Long>())))
...
//				.contentType(MediaType.APPLICATION_JSON)
//				.content(MomokoUtils.toJson(form(jwtExpirationMillis))))
//				.andExpect(status().is(200))
//				.andExpect(header().string(MomokoSecurityConfig.TOKEN_RESPONSE_HEADER_NAME, containsString(".")))
//				.andExpect(jsonPath("$.id").value(UNVERIFIED_USER_ID))
//                .andReturn();
//
//        return result.getResponse().getHeader(MomokoSecurityConfig.TOKEN_RESPONSE_HEADER_NAME);
//	}
//}
